...
### [7] МО для текстов. Определение токсичных комментариев с BERT
...
### [8] Компьютерное зрение. Определение возраста по фото
**Цель:** Обучить нейросеть для определения возраста покупателя магазина по фото, чтобы делать персонализированные предложения разным возрастным группам, а также дополнительно контролировать кассиров при продаже алкоголя.

**Пояснение:** Задача обучалась на облачном сервере Яндекса с GPU, поэтому тетрадка не содержит вызов функций.

**Стек:** pandas, matplotlib, seaborn, sklearn, tensorflow.keras

**Результат:** 
- Обучена сеть ResNet с глубиной 50 слоев для прогнозирования возраста людей по фото. Результат: средняя абсолютная ошибка модели 7.24, возможно можно улучшить метрику, используя дополнительные вычислительные мощности и время.
- Модель в среднем может ошибаться на 7.24 лет в меньшую или большую сторону, то есть: сеть с очень высокой вероятностью отнесет в нужную группу покупателей возрастом от 27 до 32, от 47 до 52, и от 67. Обращаем внимание, что модель с очень высокой вероятностью не определит ребенка или подростка в категории Зрелость и Старость, молодых в Старость и наоборот. Таким образом, рекомендации для пограничных возрастов можно сделать смешанными: например, если модель предсказывает возраст от 20 до 26, то рекомендовать товары как для подростков, так и для молодых людей и так далее. Помимо этого, если модель показывает, что покупателю 26 лет и больше, то можно с высокой вероятностью считать, что клиенту больше 18, потому что модель в среднем редко ошибается более чем на 8 лет. Рекомендуем заказчику использовать модель для автоматического определения возраста покупателей для улучшения персонализации рекомендаций и дополнительной проверки кассиров.
